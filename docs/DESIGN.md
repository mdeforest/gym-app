...
| **Set Row**            | Horizontal row: set number (tappable to toggle warm-up/normal), weight field, reps field, optional checkmark button, optional RPE badge, optional PR badge. Pre-filled from last session. Tap field to edit via numeric keypad. Swipe left to reveal delete button. Editing values propagates to subsequent incomplete sets of the same type. Checkmark is hidden when `onComplete` is nil (e.g., in history edit mode). Confirmed sets get a subtle `#FF6A3D` at 8% opacity background tint. Warm-up sets show "W" in warning color (`#FFB340`) with warm-up-specific tint when completed. RPE badge appears after checkmark when set. Gold "PR" badge appears after RPE badge when the set holds any personal record. | `Views/Components/SetRowView.swift` | Done |
...
| **Number Input**       | `#2C2C2E` background, 12pt corner radius, centered text (callout, medium weight), decimal pad keyboard. Uses a local `@State localText` buffer with `@FocusState` sync: updates the bound value on every keystroke but only syncs incoming external changes when the field is not focused (prevents cursor jumps mid-edit). `NotificationCenter.textDidBeginEditingNotification` triggers `selectAll` on focus for one-tap overwrite. | `Views/Components/NumberInputField.swift` | Done |
...
| **Equipment Section**  | List section with "Available Equipment" header and an inline "Reset" button (accent, semibold footnote) that appears whenever any equipment type is deselected. One row per `Equipment` case. Each row: 32×32pt `RoundedRectangle` icon square (accent fill when selected, tertiary fill when deselected) with a white SF Symbol, equipment display name (primary text), and a trailing `checkmark` icon (accent, bold, hidden when deselected). Full-row tap via `.contentShape(Rectangle())` and `.buttonStyle(.plain)`. Footer text explains the filter. Selecting all cases resets the `@AppStorage` value to an empty string (no filter active). | `Views/Settings/EquipmentSectionView.swift` | Done |
...
| **Exercise Detail**    | Full-height bottom sheet (`.large` detent) with drag indicator. Toolbar: star button (top-left, strength exercises only, max 10 favorites) and "Done" button. Header shows muscle group badge (`#FF6A3D` capsule fill) + custom tag. **Personal Records** section (strength only): 3-column grid showing all-time best weight, estimated 1RM, and volume with gold trophy/bolt/chart icons. How-to section: description, primary muscles (in accent color), numbered steps. Rest Timer section: "Rest Timer" headline, subtitle, horizontal Rest Time Picker pills. History section: last 5 sessions with sets/reps/weight (or time/distance for cardio), with compact gold PR badges on record-holding sets. | `Views/ExerciseLibrary/ExerciseDetailView.swift` | Done |
| **Exercise Filter Sheet** | `.medium` detent sheet with drag indicator. Two `LazyVGrid` filter sections: "Muscle Group" (4-column grid, 8 groups + "All") and "Equipment" (3-column grid, 8 types + "Any"). Each option is a full-width rounded-rect chip: accent fill + white text when selected, tertiary fill + secondary text otherwise. Toolbar: "Clear All" (left, disabled when no filters active) and "Done" (right, semibold accent). Binds to `selectedMuscleGroup` and `selectedEquipment` on the library view. | `Views/ExerciseLibrary/ExerciseFilterSheet.swift` | Done |
| **Exercise Library Toolbar** | Exercises tab navigation bar toolbar (trailing): star toggle button (`star` / `star.fill`), filter button (`line.3.horizontal.decrease.circle`) with 8pt accent dot badge when equipment or muscle group filter is active, and a `+` button (accent icon on 32pt tertiary circle). When any filter is active, an **active filter chips row** appears as a `safeAreaInset` above the list: horizontally scrolling capsule pills (accent fill, white text + xmark icon per filter) with a "Clear All" text button when ≥2 filters are active. Chips animate in/out with opacity+move transition. | `Views/ExerciseLibrary/ExerciseLibraryView.swift` | Done |
...
### Personal Records
1. When the user completes a normal (non-warm-up) strength set during an active workout, `PersonalRecordService` checks it against all historical completed sets for that exercise
2. Three record types are tracked: **Weight PR** (heaviest single-set weight), **Estimated 1RM PR** (best Epley formula result: `weight × (1 + reps / 30)`), and **Volume PR** (best single-set `weight × reps`)
3. If any PR is detected, boolean flags are stamped on the `ExerciseSet` and a gold **PR badge** appears on the set row
4. An animated **"New PR!" toast** slides down from the navigation bar area with a trophy icon, PR type labels, and a success haptic. The toast auto-dismisses after 3 seconds
5. If a completed set is un-completed (toggled off), its PR flags are cleared
6. PR badges appear throughout the app: active workout set rows, workout history detail, exercise detail history, and as trophy annotations on the strength progression chart
7. The exercise detail panel shows a **"Personal Records"** section with 3 columns: best weight, best estimated 1RM, and best volume — each with a gold icon and the record value
8. On first launch after the update, a one-time backfill walks all existing completed workouts chronologically and stamps PR flags on historical sets
...
5. **Personal Records** (strength exercises only): 3-column grid showing all-time best weight, estimated 1RM, and volume with gold icons and record values
6. **How to Perform**: Description, primary muscles (in accent color), and numbered step-by-step instructions
7. **Rest Timer**: Configurable rest duration with pill button picker (Off, 30s, 60s, 90s, 2m, 3m)
8. **Recent History**: Last 5 sessions showing date and sets (weight × reps) with gold PR badges on record-holding sets, or time/distance for cardio
9. Custom exercises show "No instructions available" in the how-to section
