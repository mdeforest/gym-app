...
| **Settings View**      | Modal `NavigationStack` with `.insetGrouped` list style. True black background. "Done" button (semibold, accent) in toolbar. Two sections: Profile (via `ProfileSectionView`) and Data Management (via `DataManagementSectionView`). ViewModel lazily initialized on appear. | `Views/Settings/SettingsView.swift` | Done |
...
3. **Export Data section**: Format picker (CSV or JSON), then "Export Workout Data" button. Tapping generates the export file via `ExportService` and presents the iOS share sheet
4. **Bottom section**: Version info row showing app version and build number. "Clear All Data" destructive button with two-step confirmation — first an alert ("This will permanently delete all workouts, templates, and custom exercises"), then a success alert after clearing
5. Clear All Data deletes all workouts, templates, and custom exercises. Built-in exercises have their `lastUsedDate` and `isFavorite` reset
6. Tap **Done** to dismiss the settings sheet
